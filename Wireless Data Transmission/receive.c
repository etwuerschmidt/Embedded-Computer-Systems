...
unsigned long parity_test = 0; //Expected parity
int recv_parity = 0; //Calculated parity
extern unsigned long last_receive;
int count = BITS_IN_TRANSMISSION;
int data3 = 0; //Current bit in bit sequence for parity calculation
int rcv_one_count = 0; //Number of ones in the received bit sequence
...
void rcv(void){
   ...
   parity_test = (Rcv1.LastValidReceived >> 31);
   parity_test &= 0x01; //Retrieve the first bit from the transmitted sequence

   //Calculate parity bit from the transmitted sequence
   while (count != 0) {
	   data3 = (last_receive >> count - 1); //Grab individual bits starting with the second (first is be parity bit)
       data3 &= 0x01;
       count -= 1; //Subtract from bits remaining

       if (data3) { //If the current bit is a 1, add one to rcv_one_count
    	   rcv_one_count += 1;
       }
   }

   if (rcv_one_count % 2 == 0) { //If number of ones in sequence is even, parity of 1
	   recv_parity = 1;
   } else { //If number of ones in sequence is odd, parity of 0
	   recv_parity = 0;
   }

   if (parity_test != recv_parity) { //If the expected parity and calculated parity are not equal, toggle the red LED to show an error
	   P1DIR |= BIT0;
   	   P1OUT |= BIT0;
   	   P2OUT |= BIT3;
   } else { //If the expected parity and calculated parity are equal, the red LED remains off
	   P1OUT &= ~BIT0;
       P2OUT &= ~BIT3;
   }
